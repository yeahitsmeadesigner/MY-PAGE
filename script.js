...
// Global portfolio page-view counter. CounterAPI stores the count remotely so it is shared across visitors.
(function initViewerCounter(){
  ...
  script.onload=async()=>{
    try{
      const counter=new Counter({workspace:'mahibalan-my-page'});
      const result=await counter.up('portfolio-page-views');
      const value=result?.value ?? result?.data ?? result;
      document.getElementById('viewer-count').textContent=Number(value).toLocaleString();
    }catch(error){
      document.getElementById('viewer-count').textContent='—';
      console.warn('Viewer counter unavailable:',error);
    }
  };
  script.onerror=()=>{document.getElementById('viewer-count').textContent='—'};
